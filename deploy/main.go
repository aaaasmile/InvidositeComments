...
import (
	...
	"github.com/aaaasmile/InvidositeComments/deploy/depl"
)
...
func main() {
	...
	rootDirRel := ".."
	pathItems := []string{"InvidositeComments.bin", "static", "templates"}
	switch *target {
	case pi3:
...
func getOutFileName(outdir string, tgt string) string {
	...
	s := fmt.Sprintf("InvidositeComments_%s_%s_%s.zip", strings.Replace(vn, ".", "-", -1), currentTime.Format("02012006-150405"), tgt) // current date-time stamp using 2006 date time format template
